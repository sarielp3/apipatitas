...
app.post('/api/regicart', (req,res) =>{
    const{peso,nomvac,fechaapl,fechasig,nombre_vac,usuario,idmascota} = req.body;
    const sql = `insert into vacunas(peso,nomvac,fechaapl, fechasig,nombre_vac,usuario,idmascota) values('${peso}','${nomvac}','${fechaapl}','${fechasig}','${nombre_vac}','${usuario}','${idmascota}')`;
    connection.query(sql,(error,results)=>{
        if(error) throw error
        else{
            res.json({status : 'registro agregado cartilla'})
        }
